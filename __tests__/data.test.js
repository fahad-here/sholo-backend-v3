...
describe('Fetching And Caching', async () => {
        ...
        (async () => {
            try {
                ...
                for (let x = 10; x <= 50; x += 10) {
                    ...
                    console.log('test', getRequiredCandlesLength(fromDate, toDate))
                    requiredTimeStamps = requiredTimeStamps + getRequiredCandlesLength(fromDate, toDate)
                    let candles = await fetchCandles(binance,  fromDate, toDate)
                    expect(moment(fromDate).valueOf()).to.be.a('number').to.equal(candles[0][0])
                    expect(moment(toDate).valueOf()).to.be.a('number').to.equal(candles[candles.length - 1][0])
                    y += 10
                    totalCandles = [...totalCandles, ...candles]
                }
                Logger.info('Required Time stamps: ' + requiredTimeStamps)
                expect(totalCandles.length).to.be.a('number').to.equal(requiredTimeStamps)
                done()
            } catch (e) {
                done(e)
            }
    ...
    it('should fetch timeframes that comes between previous ranges (11:22 - 11:42)', done => {
        (async () => {
            try {
                ...
                console.log('test', getRequiredCandlesLength(fromDate2, toDate2))
                requiredTimeStamps = getRequiredCandlesLength(fromDate2, toDate2)
                let candles2 = await fetchCandles(binance, fromDate2, toDate2)
                Logger.info('Required Time stamps: ' + requiredTimeStamps)
                expect(candles2.length).to.be.a('number').to.equal(requiredTimeStamps)
                done()
            } catch (e) {
                done(e)
            }
    ...
    it('should fetch timeframes that comes before and ends in the middle of the previously fetched candles(11:05 - 11:32)', done => {
        (async () => {
            try {
                let requiredTimeStamps
                let fromDate3 = new Date(`October 30, 2019 11:05:00`)
                let toDate3 = new Date(`October 30, 2019 11:32:00`)
                console.log('test', getRequiredCandlesLength(fromDate3, toDate3))
                requiredTimeStamps = getRequiredCandlesLength(fromDate3, toDate3)
                let candles2 = await fetchCandles(binance, fromDate3, toDate3)
                Logger.info('Required Time stamps: ' + requiredTimeStamps)
                expect(candles2.length).to.be.a('number').to.equal(requiredTimeStamps)
                done()
            } catch (e) {
    ...
    it('should fetch timeframes that start in the middle and ends after the last previously fetched candles(11:34 - 12:00)', done => {
        (async () => {
            try {
                let requiredTimeStamps
                let fromDate4 = new Date(`October 30, 2019 11:34:00`)
                let toDate4 = new Date(`October 30, 2019 12:00:00`)
                console.log('test', getRequiredCandlesLength(fromDate4, toDate4))
                requiredTimeStamps = getRequiredCandlesLength(fromDate4, toDate4)
                let candles2 = await fetchCandles(binance, fromDate4, toDate4)
                Logger.info('Required Time stamps: ' + requiredTimeStamps)
                expect(candles2.length).to.be.a('number').to.equal(requiredTimeStamps)
                done()
            } catch (e) {
